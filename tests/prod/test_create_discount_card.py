...
def test_create_cashback(page_with_video, request) -> None:
    ...
    page.locator("div").filter(has_text=re.compile(r"^Silver10005%RUB$")).get_by_role(
        "button"
    ).first.click()
    page.get_by_placeholder("100").click()
    ...
    page.locator("div").filter(
        has_text=re.compile(
            r"^Field TypeCustomField nameChoice OptionsBurger Required$"
        )
    ...
    request.node.test_info = {
        "message": f"Создана скидочная карта!\nUUID: {copied_text}"
    }
