...
def test_create_cashback(page_with_video, request) -> None:
    ...
    page.locator("div").filter(
        has_text=re.compile(r"^Field TypeCustomField nameChoice OptionsHappy Required$")
    ).get_by_role("button").nth(2).click()
    page.get_by_role("textbox", name="Enter option name").click()
    page.get_by_role("textbox", name="Enter option name").fill("Angry")
    page.locator("div").filter(
        has_text=re.compile(
            r"^Field TypeCustomField nameChoice OptionsHappySad Required$"
        )
    ...
    request.node.test_info = {"message": f"Создана карта кешбек!\nUUID: {copied_text}"}
