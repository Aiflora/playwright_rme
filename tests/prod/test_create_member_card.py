...
def test_example(page: Page) -> None:
    ...
    page.get_by_role("button", name="Icon Членская карта").click()
    page.get_by_role("textbox", name="Название карты").click()
    page.get_by_role("textbox", name="Название карты").fill("членская карта")
    page.get_by_role("button", name="Recard Me").click()
    page.get_by_label("", exact=True).nth(2).click()
    page.get_by_label("", exact=True).nth(2).fill("#216e17")
    ...
    page.get_by_label("", exact=True).nth(2).click()
    page.get_by_label("", exact=True).nth(2).fill("#256e1c")
    page.locator("[id=\"__next\"] div").filter(has_text="Выберите цвета вашей карты.Выберите цвет вашей открытки и текста. После того, ка").nth(2).click()
    ...
    page.get_by_role("textbox", name="Введите имя").fill("пол")
    page.get_by_role("textbox", name="Введите название выбора").click()
    page.get_by_role("textbox", name="Введите название выбора").fill("Ж")
    ...
    page.get_by_role("textbox", name="Введите название выбора").click()
    page.get_by_role("textbox", name="Введите название выбора").fill("М")
    page.locator("div").filter(has_text=re.compile(r"^Тип поляПользовательскоеНаименование поляВарианты выбораЖ Обязательный$")).get_by_role("button").nth(2).click()
    ...
    page.get_by_role("button", name="Empty").first.click()
    page.get_by_role("list").get_by_text("пол", exact=True).click()
    ...
    page.get_by_role("textbox", name="Opening hours").fill("тестт")
    ...
    page.get_by_role("textbox", name="Mon-Sun: 10:00 - 20:").fill("тестыы")
    ...
    if copied_text:
        ...
        print("Ошибка: UUID не скопирован. Возможно, проблема с кнопкой копирования.")
