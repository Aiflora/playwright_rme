...
def test_create_business(page_with_video, request) -> None:
    ...
    page.get_by_role("button", name="Icon Stamp and Cashback").click()
    page.get_by_text("Stamp and Cashback").click()
    page.get_by_role("textbox", name="Card Name").click()
    page.get_by_role("textbox", name="Card Name").fill("Stamp and cashback")
    page.get_by_role("button", name="Recard Me").click()

    page.get_by_label("", exact=True).nth(2).click()
    page.get_by_label("", exact=True).nth(2).fill("#d782e8")
    ...
    page.get_by_role("heading", name="Card Stamp and cashback").click()
    ...
    print(f"Создана карта штампы и кешбэк. UUID карты: {copied_text}")

    request.node.test_info = {
        "message": f"Создана карта штампы и кешбэк!\nUUID: {copied_text}"
    }
